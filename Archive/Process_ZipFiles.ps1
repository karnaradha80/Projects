...
# adds .txt extension to extension-less files, gzips each
# file, and moves the final .txt.gz to Target.
# ============================================================

# ---------- CONFIG ------------------------------------------
$ArchiveRoot   = "C:\Projects\Archive"
$FileListPath  = "$ArchiveRoot\filelist.txt"
...
foreach ($zipFileName in $fileList) {
    ...
    # 5. GZip each file to Target as <baseName>.txt.gz
    $allFiles = Get-ChildItem -Path $extractDir -Recurse -File
    $gzipFailed = $false

    foreach ($file in $allFiles) {
        $outGzName = $baseName + ".txt.gz"
        $outGzPath = Join-Path $TargetDir $outGzName

        try {
            $inStream  = [System.IO.File]::OpenRead($file.FullName)
            $outStream = [System.IO.File]::Create($outGzPath)
            ...
            $outStream.Close()
            $inStream.Close()
            Write-Host "  Created $outGzName in Target."
        }
        catch {
            Write-Warning "  Failed to gzip $($file.Name) : $_"
            $ts = Get-Date -Format "HH:mm:ss"
            Add-Content -Path $ErrorLog -Value "$ts | $zipFileName | Failed to gzip $($file.Name): $_"
            $gzipFailed = $true
        }
    }

    if ($gzipFailed) { continue }
    ...
    $ts = Get-Date -Format "HH:mm:ss"
    Add-Content -Path $SuccessLog -Value "$ts | $zipFileName | Processed OK -- $renamedCount file(s) renamed, output: $baseName.txt.gz"
